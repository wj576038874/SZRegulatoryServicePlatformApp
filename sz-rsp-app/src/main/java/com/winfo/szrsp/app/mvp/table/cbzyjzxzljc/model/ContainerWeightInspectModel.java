...
public class ContainerWeightInspectModel extends BaseModel<TableService> implements IContainerWeightInspectModel {
    public ContainerWeightInspectModel() {
        super(SzRspApplication.getContext());
    }

    @Override
    public void subData(Dialog dialog, ContainerWeightInspectData containerWeightInspectData, String qmPath,final OnSubDataListenner onSubDataListenner) {

        final String str=FastJsonUtil.beanToJsonStr(containerWeightInspectData);
        OkHttpClient client;
        ...
        mbody.addFormDataPart("ctCaseWeightInspectFile",file1.getName(), RequestBody.create(MediaType.parse("multipart/form-data"),file1));
        final RequestBody requestBody =mbody.build();
        final Request request = new Request.Builder().url(ServerReqAddress.BASE_ADDRESS+"sz/ctCaseWeightInspectRestService/insertSelective").post(requestBody).build();
...
//        String parames = FastJsonUtil.beanToJsonStr(containerWeightInspectData);
//        Observable<ResponseResult<String>> observable = mService.addJZXZLJCData(parames);
