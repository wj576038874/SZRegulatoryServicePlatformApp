 ...
 * @Package: com.winfo.szrsp.app.widget.spinner
 * @Filename: MySpinnerPopMultListArrayAdapter
 * @Author: lsj
 * @Date: 2018/3/9  11:08
 * @Description:
 * @Version:
...
public class MySpinnerPopMultListArrayAdapter extends ArrayAdapter<SpinnearModel> {
    ...
    private void setSelectedState(CheckedTextView view, boolean selected){
        ...
        if(selected){
            ...
            Drawable drawable = ContextCompat.getDrawable(mcontext, R.mipmap.checkbox_pressed);
            ...
            //设置CheckedTextView控件的android:drawableTop属性值
            Drawable drawable = ContextCompat.getDrawable(mcontext, R.mipmap.checkbox_normal);
            //setCompoundDrawables 画的drawable的宽高是按drawable.setBound()设置的宽高
            //而setCompoundDrawablesWithIntrinsicBounds是画的drawable的宽高是按drawable固定的宽高，即通过getIntrinsicWidth()与getIntrinsicHeight()自动获得
            drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
            view.setCompoundDrawables(null, null, drawable, null);
