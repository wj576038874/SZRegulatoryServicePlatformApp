...
public class CertificateNationalShipFragment extends Fragment implements ICertificateActvity,View.OnClickListener{
    ...
    private void initEvent() {
        ...
        mRefreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(RefreshLayout refreshlayout) {
                presenter.loadCertificateData(ais.getID(),"nationalShipCertificate",false);
            }
        });
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.layout_faile:
                presenter.loadCertificateData(ais.getID(),"nationalShipCertificate",true);
                break;
        }
    }
    ...
    public void OnSucess(CertificateInfo certificateInfo) {
        ...
        if(certificateInfo.getNationalShipCertificate()!=null&&certificateInfo.getNationalShipCertificate().size()>0){
            noticeCerMain.noticeCerMain("nationalShipCertificate",false);
            mRefreshLayout.setVisibility(View.VISIBLE);
            faile_view.setVisibility(View.GONE);
            adapter = new CertificateAdapter(getActivity(), certificateInfo.getNationalShipCertificate(), R.layout.certificate_item);
            gridView.setAdapter(adapter);
        }else {
            noticeCerMain.noticeCerMain("nationalShipCertificate",true);
