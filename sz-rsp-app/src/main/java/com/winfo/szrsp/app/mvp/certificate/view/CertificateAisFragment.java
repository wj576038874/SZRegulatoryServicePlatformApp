...
public class CertificateAisFragment extends Fragment implements ICertificateActvity,View.OnClickListener{
    ...
    public void OnSucess(CertificateInfo certificateInfo) {
        ...
        if(certificateInfo.getAis()!=null&&certificateInfo.getAis().size()>0){
            noticeCerMain.noticeCerMain("ais",false);
            mRefreshLayout.setVisibility(View.VISIBLE);
            faile_view.setVisibility(View.GONE);

            adapter = new CertificateAdapter(getActivity(), certificateInfo.getAis(), R.layout.certificate_item);
            gridView.setAdapter(adapter);
        }else {
            noticeCerMain.noticeCerMain("ais",true);
